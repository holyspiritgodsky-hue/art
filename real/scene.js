...
const shipExteriorImg = new Image();
const earthZoneImgs = {
...
let shipExteriorImgLoaded = false;
const SHIP_SCENE_ENABLED = true;
const shipCabinWalkRects = [
    { x: -0.88, y: -0.24, w: 1.76, h: 0.48 },
    { x: -0.34, y: -0.52, w: 0.68, h: 0.24 },
    { x: -0.34, y: 0.28, w: 0.68, h: 0.24 },
    { x: 0.72, y: -0.18, w: 0.22, h: 0.36 },
    { x: -0.96, y: -0.18, w: 0.22, h: 0.36 }
];
...
shipExteriorImg.onload = () => { shipExteriorImgLoaded = true; };
worldMapImg.onerror = () => {
...
assignLocalImage(shipExteriorImg, 'img/lightship.jpg');
...
function ensureShipWorkZonesInitialized() {
    if (!SHIP_SCENE_ENABLED) return;
    if (earthWorkZones.length === 0) {
        earthWorkZones.push(
            { id: 'engine_room', role: 'company', tier: 'slum', label: '引擎工位', x: -0.90, y: -0.12, w: 0.24, h: 0.24, color: 'rgba(251,146,60,0.24)', border: 'rgba(253,186,116,0.92)' },
            { id: 'life_support', role: 'company', tier: 'slum', label: '维生系统', x: -0.62, y: -0.12, w: 0.24, h: 0.24, color: 'rgba(16,185,129,0.22)', border: 'rgba(110,231,183,0.92)' },
            { id: 'school', role: 'school', label: '训练舱', x: -0.12, y: -0.12, w: 0.24, h: 0.24, color: 'rgba(59,130,246,0.22)', border: 'rgba(147,197,253,0.92)' },
            { id: 'command_tower', role: 'tower', label: '舰桥指挥台', x: 0.72, y: -0.12, w: 0.22, h: 0.24, color: 'rgba(244,63,94,0.2)', border: 'rgba(251,113,133,0.92)' },
            { id: 'moon_gravity_station', role: 'moon_station', label: '跃迁门', x: -0.10, y: -0.44, w: 0.20, h: 0.16, color: 'rgba(56,189,248,0.2)', border: 'rgba(125,211,252,0.92)' }
        );
    }
    if (earthPreludeZones.length === 0) {
        earthPreludeZones.push(
            { id: 'time_machine', role: 'time_machine', label: '时空折叠台', x: -0.14, y: -0.14, w: 0.28, h: 0.28, color: 'rgba(148,163,184,0.25)', border: 'rgba(203,213,225,0.92)' }
        );
    }
}

function clampPointToShipWalkway(localX, localY, earthR, playerSize) {
    if (!SHIP_SCENE_ENABLED) {
        return { x: localX, y: localY };
    }
    const safeR = Math.max(8, Number(earthR) || 8);
    const margin = Math.max(0.02, Math.min(0.22, (Number(playerSize) || 12) / safeR / 2));
    const nx = localX / safeR;
    const ny = localY / safeR;
    let bestX = nx;
    let bestY = ny;
    let bestD2 = Number.POSITIVE_INFINITY;
    let inside = false;

    for (let i = 0; i < shipCabinWalkRects.length; i += 1) {
        const rect = shipCabinWalkRects[i];
    ...
    if (!inside && !Number.isFinite(bestD2)) {
        return { x: localX, y: localY };
    }
    return { x: bestX * safeR, y: bestY * safeR };
}
...
function getShipGeometry(ex, ey, earthR) {
    const bodyW = earthR * 3.02;
    const bodyH = earthR * 1.18;
    const bodyX = ex - bodyW / 2;
    const bodyY = ey - bodyH / 2;
    const noseW = earthR * 0.7;
    const tailW = earthR * 0.5;
    const cabinW = earthR * 1.64;
    const cabinH = earthR * 0.74;
    const cabinX = ex - cabinW / 2;
    const cabinY = ey - cabinH / 2;
    const canopyW = earthR * 0.92;
    const canopyH = earthR * 0.24;
    const canopyX = ex - canopyW / 2;
    const canopyY = ey - cabinH * 0.5 - canopyH * 0.35;
    return {
        bodyX,
        bodyY,
        bodyW,
...
function drawShipEarthScene(ex, ey, earthR, now) {
    const geo = getShipGeometry(ex, ey, earthR);
    const leftX = geo.bodyX;
    const rightX = geo.bodyX + geo.bodyW;
    const topY = geo.bodyY;
    const bottomY = geo.bodyY + geo.bodyH;
    const noseInset = earthR * 0.54;
    const shoulderX = leftX + earthR * 0.3;
    const railTop = topY + earthR * 0.11;
    const railBottom = bottomY - earthR * 0.11;
    const traceHullPath = () => {
        ctx.beginPath();
        ctx.moveTo(leftX + earthR * 0.06, ey);
        ctx.lineTo(shoulderX, railTop);
        ctx.lineTo(rightX - noseInset, topY + earthR * 0.04);
        ctx.lineTo(rightX + geo.noseW, ey);
        ctx.lineTo(rightX - noseInset, bottomY - earthR * 0.04);
        ctx.lineTo(shoulderX, railBottom);
        ctx.closePath();
    };

    const hullGlow = ctx.createRadialGradient(ex, ey, earthR * 0.38, ex, ey, earthR * 1.9);
    hullGlow.addColorStop(0, 'rgba(110,231,255,0.14)');
    hullGlow.addColorStop(1, 'rgba(2,6,23,0)');
    ctx.fillStyle = hullGlow;
    ctx.beginPath();
    ctx.ellipse(ex, ey, earthR * 1.86, earthR * 0.92, 0, 0, Math.PI * 2);
    ctx.fill();

    // Main hull
    traceHullPath();
    const hullGrad = ctx.createLinearGradient(geo.bodyX, geo.bodyY, geo.bodyX, geo.bodyY + geo.bodyH);
    hullGrad.addColorStop(0, 'rgba(71,85,105,0.94)');
    hullGrad.addColorStop(0.5, 'rgba(15,23,42,0.97)');
    hullGrad.addColorStop(1, 'rgba(2,6,23,0.99)');
    ctx.fillStyle = hullGrad;
    ctx.fill();
    ctx.lineWidth = Math.max(1.4, earthR * 0.025);
    ctx.strokeStyle = 'rgba(148,163,184,0.78)';
    ctx.stroke();

    // Hard chin lines
    ctx.beginPath();
    ctx.moveTo(leftX + earthR * 0.44, topY + earthR * 0.2);
    ctx.lineTo(rightX - earthR * 0.64, topY + earthR * 0.12);
    ctx.moveTo(leftX + earthR * 0.44, bottomY - earthR * 0.2);
    ctx.lineTo(rightX - earthR * 0.64, bottomY - earthR * 0.12);
    ctx.lineWidth = Math.max(1, earthR * 0.012);
    ctx.strokeStyle = 'rgba(203,213,225,0.26)';
    ctx.stroke();

    // Nose tip armor
    ctx.beginPath();
    ctx.moveTo(rightX - earthR * 0.2, ey - earthR * 0.18);
    ctx.lineTo(rightX + geo.noseW, ey);
    ctx.lineTo(rightX - earthR * 0.2, ey + earthR * 0.18);
    ctx.closePath();
    const noseGrad = ctx.createLinearGradient(rightX - earthR * 0.2, ey, rightX + geo.noseW, ey);
    noseGrad.addColorStop(0, 'rgba(100,116,139,0.92)');
    noseGrad.addColorStop(1, 'rgba(30,41,59,0.98)');
    ...
    // Rear module
    ctx.beginPath();
    ctx.moveTo(leftX - geo.tailW, ey - earthR * 0.2);
    ctx.lineTo(leftX, ey - earthR * 0.3);
    ctx.lineTo(leftX, ey + earthR * 0.3);
    ctx.lineTo(leftX - geo.tailW, ey + earthR * 0.2);
    ctx.closePath();
    const tailGrad = ctx.createLinearGradient(leftX - geo.tailW, ey, leftX, ey);
    tailGrad.addColorStop(0, 'rgba(2,6,23,0.98)');
    tailGrad.addColorStop(1, 'rgba(30,41,59,0.96)');
    ctx.fillStyle = tailGrad;
    ctx.fill();
    ctx.strokeStyle = 'rgba(100,116,139,0.72)';
    ctx.stroke();

    if (shipExteriorImgLoaded) {
        ctx.save();
        traceHullPath();
        ctx.clip();
        ctx.globalAlpha = 0.16;
        ctx.drawImage(shipExteriorImg, geo.bodyX, geo.bodyY, geo.bodyW, geo.bodyH);
        ctx.restore();
    }
    ...
    ctx.beginPath();
    ctx.moveTo(ex - earthR * 0.6, ey - earthR * 0.02);
    ctx.lineTo(ex - earthR * 0.16, ey - earthR * 0.56);
    ctx.lineTo(ex + earthR * 0.28, ey - earthR * 0.24);
    ctx.lineTo(ex + earthR * 0.02, ey - earthR * 0.02);
    ctx.closePath();
    ctx.fill();
    ctx.beginPath();
    ctx.moveTo(ex - earthR * 0.6, ey + earthR * 0.02);
    ...
    // Cabin core
    drawRoundRectPath(geo.cabinX, geo.cabinY, geo.cabinW, geo.cabinH, Math.max(8, earthR * 0.12));
    const cabinGrad = ctx.createLinearGradient(geo.cabinX, geo.cabinY, geo.cabinX, geo.cabinY + geo.cabinH);
    cabinGrad.addColorStop(0, 'rgba(12,30,58,0.9)');
    cabinGrad.addColorStop(1, 'rgba(5,10,22,0.94)');
    ...
    // Cockpit canopy
    drawRoundRectPath(geo.canopyX, geo.canopyY, geo.canopyW, geo.canopyH, Math.max(6, earthR * 0.09));
    const canopyGrad = ctx.createLinearGradient(geo.canopyX, geo.canopyY, geo.canopyX + geo.canopyW, geo.canopyY + geo.canopyH);
    canopyGrad.addColorStop(0, 'rgba(59,130,246,0.22)');
    canopyGrad.addColorStop(0.5, 'rgba(125,211,252,0.44)');
    canopyGrad.addColorStop(1, 'rgba(30,64,175,0.2)');
    ctx.fillStyle = canopyGrad;
    ctx.fill();
    ctx.strokeStyle = 'rgba(186,230,253,0.72)';
    ctx.lineWidth = Math.max(1, earthR * 0.014);
    ctx.stroke();

    ctx.save();
    drawRoundRectPath(geo.cabinX + 1, geo.cabinY + 1, geo.cabinW - 2, geo.cabinH - 2, Math.max(7, earthR * 0.11));
    ctx.clip();
    const lineGap = Math.max(8, earthR * 0.1);
    const phase = (now * 0.05) % lineGap;
    ctx.fillStyle = 'rgba(148,163,184,0.12)';
    for (let x = geo.cabinX + phase; x < geo.cabinX + geo.cabinW + lineGap; x += lineGap) {
        ctx.fillRect(x, geo.cabinY, Math.max(1, earthR * 0.008), geo.cabinH);
    }
    ctx.restore();

    // Rear engines with animated exhaust
    const exhaustPulse = 0.7 + Math.sin(now * 0.016) * 0.3;
    const engineY = [ey - earthR * 0.2, ey, ey + earthR * 0.2];
    engineY.forEach((y, idx) => {
        const exNozzle = geo.bodyX - geo.tailW - earthR * 0.07;
        ctx.beginPath();
        ctx.ellipse(exNozzle, y, earthR * 0.07, earthR * 0.044, 0, 0, Math.PI * 2);
        ctx.fillStyle = 'rgba(148,163,184,0.84)';
        ctx.fill();
        ctx.strokeStyle = 'rgba(30,41,59,0.78)';
        ctx.lineWidth = Math.max(1, earthR * 0.01);
        ctx.stroke();

        const flameW = earthR * (0.2 + exhaustPulse * 0.16 - idx * 0.01);
        const flame = ctx.createLinearGradient(exNozzle - flameW, y, exNozzle, y);
        flame.addColorStop(0, 'rgba(34,211,238,0)');
        flame.addColorStop(0.45, 'rgba(34,211,238,0.55)');
        flame.addColorStop(1, 'rgba(186,230,253,0.82)');
        ctx.fillStyle = flame;
        ctx.beginPath();
        ctx.moveTo(exNozzle, y - earthR * 0.04);
        ctx.lineTo(exNozzle - flameW, y);
        ctx.lineTo(exNozzle, y + earthR * 0.04);
        ctx.closePath();
        ctx.fill();
    });
    ...
    ctx.lineWidth = Math.max(1, earthR * 0.01);
    ctx.beginPath();
    ctx.moveTo(leftX + earthR * 0.22, ey);
    ctx.lineTo(rightX + earthR * 0.2, ey);
    ctx.stroke();
}
...
function ensureEarthWorkState() {
    const g = window.game;
    if (!g) return;
    ensureShipWorkZonesInitialized();
...
function setEarthWalkerTargetFromScreen(planet, hitX, hitY) {
    ...
    const clamped = clampPointToShipWalkway(localX, localY, earthR, playerSize);
    localX = clamped.x;
    localY = clamped.y;
...
function updateEarthWalkerAndOutput(now, ex, ey, earthR) {
    ...
    const confined = clampPointToShipWalkway(earthWalker.localX, earthWalker.localY, earthR, playerSize);
    earthWalker.localX = confined.x;
    earthWalker.localY = confined.y;
...
function drawSolarSystem() {
    ...
    if(earthR>=1){
        ...
        if (SHIP_SCENE_ENABLED) {
            drawShipEarthScene(ex, ey, earthR, now);
        } else {
